...
function Data:CalcPrices(craft, mode)
	...
	for id, matQuantity in pairs(craft.mats) do
		...
		Data[mode].mats[id].cost = Data[mode].mats[id].cost or 1
		cost = cost + matQuantity*Data[mode].mats[id].cost
	end
...
function Data:BuildCraftQueue(queueType)
	...
	if queueType == "restock" then
		for itemID, data in pairs(Data[mode].crafts) do
			if data.enabled and not data.hasCD then
				...
				elseif pMethod == "percent" then
					local cost, buyout, profit = Data:CalcPrices(data, TSM.Crafting.mode)
		...
		local sortedData = Data:GetSortedData(Data[mode].crafts, function(a, b)
				...
				local acost, _, aprofit = Data:CalcPrices(a, TSM.Crafting.mode)
				local bcost, _, bprofit = Data:CalcPrices(b, TSM.Crafting.mode)
		...
		for _, sData in pairs(sortedData) do
			...
			local profit = 0
			if data then
				profit = select(3, Data:CalcPrices(itemID, TSM.Crafting.mode)) or profit
			end
...
function Data:ScanCrafts()
	...
	for index=2, GetNumTradeSkills() do
		...
		elseif not TSM:IsEnchant(tsLink) then
			dataTemp.spellID = TSMAPI:GetItemID(GetTradeSkillRecipeLink(index))
			if tsLink and dataTemp.spellID then 
		...
		if dataTemp.name then
			...
			dataTemp.hasCD = select(2, GetTradeSkillCooldown(index))
			
			dataTemp.group = TSM[TSM.mode]:GetSlot(dataTemp.itemID)
			if not dataTemp.group then
				if TSM.mode == "Inscription" then
					dataTemp.group = #(TSM.Inscription:GetSlotList())
				else
					dataTemp.group = #TSM[TSM.mode].slotList
				end
			end
			...
			if valid then
				...
				if not TSM.Data[TSM.mode].crafts[itemID] then
					...
					TSM.Data[TSM.mode].crafts[itemID].enabled = not TSM.db.profile.lastScan[TSM.mode] or TSM.db.profile.enableNewTradeskills
				else
