...
function Crafting:UpdateQueuing()
	...
	if numPairs == topIndex then
		...
		for group, crafts in ipairs(sortedGroupedData) do
			...
			for _, sData in pairs(crafts) do
				...
				data.name = data.name
				data.buyout, data.profit = select(2, TSM.Data:CalcPrices(item, Crafting.mode))
				data.itemID = itemID
	...
	for index, data in pairs(allRows) do
		...
		if index >= offset and displayIndex < MAX_ROWS_QUEUING then
			...
			local cost, buyout, profit = TSM.Data:CalcPrices(item, Crafting.mode)
			...
			if buyout and profit then
				...
				local formattedBSilver = "|cffeeeeee" .. floor(bSilver) .. "s|r"
				if bSilver == 0 then
					formattedBSilver = ""
				end
				buyout = (specialBuyout and CYAN or WHITE) .. floor(bGold) .. "|r" .. GOLD_TEXT .. formattedBSilver
				if profit > 0 then
					profit = GREEN .. floor(profit) .. "|r" .. GOLD_TEXT .. percentText
				else
					profit = RED .. floor(profit) .. "|r" .. GOLD_TEXT .. percentText
				end
			else
				buyout = nil
				profit = nil
...
function Crafting:CreateCraftRows()
	...
	for count=1, MAX_ROWS_CRAFTING do
		...
		-- local backdrop = {bgFile="Interface\\Buttons\\White8x8", tile=true}
		-- row:SetBackdrop(backdrop)
		-- if count%2 == 1 then
			-- row:SetBackdropColor(51/255, 61/255, 33/255, 0.4)
		-- else
			-- row:SetBackdropColor(12/255, 45/255, 62/255, 0.4)
		-- end
...
function Crafting:CreateShoppingRows()
	...
	for count=1, MAX_ROWS_SHOPPING do
		...
		-- local backdrop = {bgFile="Interface\\Buttons\\UI-Listbox-Highlight", tile=false}
		-- row:SetBackdrop(backdrop)
		-- if count%2 == 1 then
			-- row:SetBackdropColor(255/255, 165/255, 79/255, 0.4)
		-- else
			-- row:SetBackdropColor(165/255, 255/255, 165/255, 0.4)
		-- end
...
function Crafting:CreateQueuingTitleRow()
	...
	row.buyout = Crafting:CreateTitleButton(row, L["Market Value"], 315, 410)
...
function Crafting:GetQueueItemOrder(item, itemID)
	if TSM.db.global.queueSort == "buyout" then
		return select(2, TSM.Data:CalcPrices(item, Crafting.mode)) or 1/0
	...
	elseif TSM.db.global.queueSort == "profitPercent" then
		...
		return select(3, TSM.Data:CalcPrices(item, Crafting.mode)) or 1/0
	end
end