...
function GUI:DrawMaterials(container)
	...
	for num=1, #(matList) do
		...
		tinsert(inline, {
				type = "EditBox",
				value = tostring(TSM.Data[TSM.mode].mats[matList[num]].cost),
				relativeWidth = 0.15,
				disabled = not TSM.db.profile.matLock[matList[num]],
				callback = function(self,_,value)
						value = tonumber(value)
						if value and (value < 1000000) then
							TSM.Data[TSM.mode].mats[matList[num]].cost = value
						else
							self:SetText(0)
						end
					end,
		...
		tinsert(inline, {
				type = "InteractiveLabel",
				text = select(2, GetItemInfo(matList[num])) or TSM.Data[TSM.mode].mats[matList[num]].name,
				relativeWidth = 0.35,
...
function GUI:UpdateQueue(forced, mode)
	if not forced and mode == updateThrottle.lastProfession then
		...
		local function FillQueue(craftID, quantity, level)
			if not TSM.Data[mode].crafts[craftID] or not TSM.Data[mode].crafts[craftID].enabled or level > 3 then return end
		...
		local toDo = {}
		for itemID, data in pairs(TSM.Data[mode].crafts) do
			if data.queued > 0 and data.enabled and not data.hasCD then
				tinsert(toDo, data)
			end
		end
		...
		local altTime, orderTime = 0, 0
		
		local orderCache = {}
		for itemID, data in pairs(TSM.Data[mode].crafts) do
			local t1 = GetTime()
			if data.intermediateQueued and data.enabled and not data.hasCD then
				local bags, bank, auctions = TSM.Data:GetPlayerNum(itemID)
				local numHave = TSM.Data:GetAltNum(itemID) + bags + bank + auctions
				data.queued = data.queued - numHave
				data.intermediateQueued = data.intermediateQueued - numHave
				if data.queued < 0 then data.queued = 0 end
				if data.intermediateQueued < 0 then data.intermediateQueued = nil end
				for matID, quantity in pairs(data.mats) do
					FillQueue(matID, quantity*data.queued, 1)
				end
			end
			altTime = altTime + (GetTime() - t1)
			if data.queued > 0 then -- find crafts that are queued
				...
				orderTime = orderTime + (GetTime() - t1)
				if partial and order ~= 3 then
					orderCache[iSpellID] = 2.5
				else
					orderCache[iSpellID] = order
				end
