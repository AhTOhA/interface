...
local function SU_SummonShow(self,hide) -- with hide set, will force the frame to hide, even in 'combat'
	...
	if FW.Settings.SummonEnable and not hide and FW.GROUPED and not WorldMapFrame:IsShown() then
		FWSUFrame:Show();
		FWSUBackground:Show();
...
local function SU_ScanSummon()
	...
	if FW.Settings.SummonEnable and FW.GROUPED and not InCombatLockdown() and not WorldMapFrame:IsShown() then -- dont do shit if not in raid, in combat or when your map is shown :P
		SU_ClearSummon();
		-- Do scans
		SU_StoreMap();
...
local function SU_SummonWhisper(event,arg1,arg2)
	arg1 = strlower(arg1);
	if not FW.GROUPED or not FW.Settings.SummonEnable or not FW.Settings.SummonKeyword or strfind(arg1,strlower(FWL.SUMMON_REQUEST_BLOCK)) then return; end
...
FW:SetMainCategory(FWL.ADVANCED,FW.ICON.DEFAULT,99,"DEFAULT");
	FW:SetSubCategory(FWL.SUMMON_ASSISTANT,FW.ICON.DEFAULT,8);
		FW:RegisterOption(FW.MS0,1,FW.NON,FWL.FRAME_LEVEL,FWL.FRAME_LEVEL_TT,	"SummonStrata", SU_SummonShow);
