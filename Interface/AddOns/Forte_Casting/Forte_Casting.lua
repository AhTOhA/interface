...
local hastebuffs = {
	...
	["DRUID"] = {
		[FW:SpellName(16886)] = 1.20, -- Nature's Grace
	},
	["SHAMAN"] = {
		[FW:SpellName(64701)] = 1.20, -- Elemental Mastery
	},
	["Troll"] = {
...
local function CA_StatsUpdate()
	...
	if CA_SpellHaste ~= newhaste then
	...
	if not CA_IsChannel[spellName] then -- if marked as special channel it's already taken care off
		...
		if ST and ST.Drain[spellName] then
			...
			if i then
				...
				ST.ST:insert(endTime,FW:CastTime(spell),realduration,tar,0,ST.DRAIN,texture,spellName,targettype,0,guid,0,id,0,1,0,0,"filter",icon,endTime,realduration,1.0,0);
			end
		end
		-- *** end new drain code ***
		if target then
...
function CA:HasteFactor(s) -- spell, rank, combopoints, unit type (2=players)
	-- duration adjustments
	if ST and ST.Track[s] then
		-- always hasted
		if ST.Track[s][8] ~= 0 then
...
function CA:Duration(s,p,u) -- RETURNS DURATION UNAFFECTED BY HASTE spell, combopoints, unit type (2=players)
	...
	if ST and ST.Track[s] then
		...
		-- % change based on talents
		if ST.Track[s]["tp"] then
			for k, v in pairs(ST.Track[s]["tp"]) do
				if v[FW.Talent[k]] then
					dura = dura * v[FW.Talent[k]];
				end
			end
		...
		if u == 2 and ST.Track[s][5] ~= 0 and dura>ST.Track[s][5] then dura=ST.Track[s][5] end 
	end
	return dura;
end
...
local function CA_SelfEnd(arg2)
	...
	while i <= CA_SelfQueue.rows do
		...
		if CA_SelfQueue[i][4]~=1 and (not arg2 or spell==arg2) then
			...
			if ST and ST.Track[s] and ST.Track[s][1] == 1 then
				CA_SelfQueue[i][2] = 1;
...
local function CA_SelfSent(spell,rank,target)
	local s = (ST and ST.SpellRename[spell]) or spell;
	--FW:Debug("Spell Sent");
	LastCast = GetTime();
	CA_SelfChannelEnd();
