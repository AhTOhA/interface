...
function module:SetFPS()
	...
	local function Update(self, t)
		int = int - t
		if int < 0 then
			Text_fps:SetFormattedText("%dfps   %dms", floor(GetFramerate()), select(3, GetNetStats()))
			int = 1
		end		
	end
...
function module:SetDPS(refresh)
	...
	function Stat11:COMBAT_LOG_EVENT_UNFILTERED(_, eventType, Id, _, _, TargetId, _, _, spellID, spellName, _, amount, amount2)
		local record = false
