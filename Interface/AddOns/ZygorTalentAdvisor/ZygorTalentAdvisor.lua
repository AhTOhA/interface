...
function me:PREVIEW_TALENT_PRIMARY_TREE_CHANGED()
	self:UpdateSuggestions(false)
	ZygorTalentAdvisorPopout_Update()
end

function me:USE_GLYPH()
	self:UpdateSuggestions(false)
	ZygorTalentAdvisorPopout_Update()
end
...
function me:CacheGlyphAvailability()
	self.glyphsAvailable={}
	if not GlyphFrame then return end
	for i=1,50 do
		local name,gtype,avail,icon,spell = GetGlyphInfo(i)
		if name and name~="header" then
			self.glyphsAvailable[spell] = {name=name,gtype=gtype,avail=avail,icon=icon,spell=spell}
			self.glyphsAvailable[name] = self.glyphsAvailable[spell]
		end
	end

	for socket=1,NUM_GLYPH_SLOTS do
		local enabled, glyphType, glyphTooltip, glyphSpell, icon = GetGlyphSocketInfo(socket)
		if glyphSpell then self.glyphsAvailable[glyphSpell].inscribed = true end
	end
end

local socketinfo_to_glyphinfo = {2,3,1}
function me:GetGlyphSuggestions()
	...
	for i=1,glyphs and #glyphs or 0 do
		...
		local glyph = self.glyphsAvailable[name]
...
function me:GetBuildStatus(build,pet,preview)
	...
	-- bail out if the specs don't match
...
function me:GetStatusMessage(pet,glyphs)
	if glyphs then
