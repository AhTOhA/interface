...
local fontn = [=[Interface\AddOns\LUI\media\fonts\Russel Square LT.ttf]=]
local fontlol = [=[Interface\Addons\LUI\media\fonts\vibrocen.ttf]=]
...
local function SetStyle(self, unit)
	...
	if unit == "partytarget" or unit == "partypet" then
		self:ClearAllPoints()
	...
	if unit == "party" or unit == "partytarget" then
	...
	if oufdb.Aura.buffs_enable then
		...
		if unit ~= "partytarget" then self.Buffs.onlyShowPlayer = oufdb.Aura.buffs_playeronly end
	...
	if oufdb.Aura.debuffs_enable then
		...
		if unit == "partytarget" then self.Debuffs.onlyShowPlayer = oufdb.Aura.debuffs_playeronly end
	...
	self.Threat.Enable = oufdb.Border.Aggro
	...
	self.Health.value.colorIndividual = oufdb.Texts.Health.IndividualColor.b
	...
	if unit == "partytarget" then
		...
		if db.oUF.Settings.show_v2_party_textures == true then self.CreateV2Textures() end
	end
...
oUF:RegisterStyle("Party", SetStyle)
oUF:SetActiveStyle("Party")

if db.oUF.Party.Enable == false and db.oUF.Party.UseBlizzard == false then
...
if not db.oUF.Party.Enable == true then return end
...
local initialconfigfunc = [[
	local unit = ...
	if unit == "party" then
		self:SetHeight(]]..tonumber(db.oUF.Party.Height)..[[)
		self:SetWidth(]]..tonumber(db.oUF.Party.Width)..[[)
	elseif unit == "partytarget" then
		self:SetHeight(]]..tonumber(db.oUF.PartyTarget.Height)..[[)
		self:SetWidth(]]..tonumber(db.oUF.PartyTarget.Width)..[[)
	elseif unit == "partypet" then
		self:SetHeight(]]..tonumber(db.oUF.PartyPet.Height)..[[)
		self:SetWidth(]]..tonumber(db.oUF.PartyPet.Width)..[[)
	end
]]

if db.oUF.PartyTarget.Enable == true then
	if db.oUF.PartyPet.Enable == true then
		temp = "oUF_LUI_partyboth"
	else
		temp = "oUF_LUI_partytarget"
	end
else
	if db.oUF.PartyPet.Enable == true then
		temp = "oUF_LUI_partypet"
	else
		temp = nil
	end
end
...
if temp ~= nil then
	party = oUF:SpawnHeader("oUF_LUI_party", nil, nil,
		"showParty", true,
