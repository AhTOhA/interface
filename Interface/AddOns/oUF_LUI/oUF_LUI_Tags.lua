...
oUF.TagEvents['NameTank'] = 'UNIT_NAME_UPDATE'
if (not oUF.Tags['NameTank']) then
	oUF.Tags['NameTank'] = function(unit)
		local name = UnitName(unit)
		if name then
			if unit == 'pet' and name == 'Unknown' then
				return 'Pet'
...
if (not oUF.Tags['druidmana2']) then
	oUF.Tags['druidmana2'] = function(unit)
		...
		if db.oUF.Player.Texts.DruidMana.ColorClass then
			final = format("|cff%02x%02x%02x%s|r",color[1]*255,color[2]*255,color[3]*255,final)
		elseif db.oUF.Player.Texts.DruidMana.ColorType then
			final = format("|cff%02x%02x%02x%s|r",color2[1]*255,color2[2]*255,color2[3]*255,final)
		elseif db.oUF.Player.Texts.DruidMana.IndividualColor.Enable then
			final = format("|cff%02x%02x%02x%s|r",db.oUF.Player.Texts.DruidMana.IndividualColor.r*255,db.oUF.Player.Texts.DruidMana.IndividualColor.g*255,db.oUF.Player.Texts.DruidMana.IndividualColor.b*255,final)
		end
