...
function clcprot:SWING_COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, combatEvent, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
...
function clcprot:UpdateSwingBar()
...
function clcprot:InitSwingBar()
	local frame = CreateFrame("Frame", "clcprotswingBar", clcprotFrame)
	frame:Hide()
...
function clcprot:UpdateSwingBarLayout()
	...
	swingBar:SetPoint(opt.point, clcprotFrame, opt.pointParent, opt.x, opt.y)			
...
-- toggle on and off
function clcprot:ToggleSwingTimer()
