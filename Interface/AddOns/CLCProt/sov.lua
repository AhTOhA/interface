local function bprint(...)
	...
	DEFAULT_CHAT_FRAME:AddMessage("CLCProt.sovTracking: " .. table.concat(t, " "))
end
...
function clcprot:SOV_COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, combatEvent, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellId, spellName, spellSchool, spellType, dose, ...)
...
function clcprot:Sov_SPELL_AURA_APPLIED(guid, name, dose)
	dose = dose or 1
...
function clcprot:Sov_SPELL_AURA_APPLIED_DOSE(guid, name, dose)
	for i = 1, MAX_SOVBARS do
	...
	clcprot:Sov_SPELL_AURA_APPLIED(guid, name, dose)
end

-- updates the stack for the guid if it founds it
function clcprot:Sov_SPELL_AURA_REMOVED_DOSE(guid, name, dose)
	for i = 1, MAX_SOVBARS do
		if sovBars[i].guid == guid then
			sovBars[i].labelStack:SetText(dose)
			sovBars[i].active = true
...
-- refreshes the timer
function clcprot:Sov_SPELL_AURA_REFRESH(guid, name)
	for i = 1, MAX_SOVBARS do
		if sovBars[i].guid == guid then
			sovBars[i].start = GetTime()
			sovBars[i].active = true
	...
	-- not found, but try to apply it
	clcprot:Sov_SPELL_AURA_APPLIED(guid, name, 5)
end

-- deactivates the bar
function clcprot:Sov_SPELL_AURA_REMOVED(guid)
	for i = 1, MAX_SOVBARS do
		if sovBars[i].guid == guid then
...
-- update the bars
function clcprot:UpdateSovBars()
...
function clcprot:UpdateSovBar(index)
...
-- updates everything
function clcprot:UpdateSovBarsLayout()
	local opt = db.sov
	...
	if opt.useButtons then
		clcprotSovAnchor:SetWidth(opt.height)
		clcprotSovAnchor:SetHeight(opt.height)
		clcprotSovAnchor:ClearAllPoints()
		clcprotSovAnchor:SetPoint(opt.point, clcprotFrame, opt.pointParent, opt.x, opt.y)
	else
		clcprotSovAnchor:SetWidth(opt.width)
		clcprotSovAnchor:SetHeight(opt.height)
		clcprotSovAnchor:ClearAllPoints()
		clcprotSovAnchor:SetPoint(opt.point, clcprotFrame, opt.pointParent, opt.x, opt.y)
	end
	...
	for i = 1, MAX_SOVBARS do
		...
		if opt.useButtons then
			...
			if opt.growth == "up" then
				bar:SetPoint("BOTTOM", clcprotSovAnchor, "BOTTOM", 0, (i - 1) * (opt.height + opt.spacing))
			elseif opt.growth == "left" then
				bar:SetPoint("LEFT", clcprotSovAnchor, "LEFT", (1 - i) * (opt.height + opt.spacing), 0)
			elseif opt.growth == "right" then
				bar:SetPoint("RIGHT", clcprotSovAnchor, "RIGHT", (i - 1) * (opt.height + opt.spacing), 0)
			else
				bar:SetPoint("TOP", clcprotSovAnchor, "TOP", 0, (1 - i) * (opt.height + opt.spacing) )
			end
			...
			-- positioning
			if opt.growth == "up" then
				bar:SetPoint("BOTTOM", clcprotSovAnchor, "BOTTOM", opt.height / 2, (i - 1) * (opt.height + opt.spacing))
			elseif opt.growth == "left" then
				bar:SetPoint("LEFT", clcprotSovAnchor, "LEFT", (1 - i) * (opt.width + opt.spacing) + opt.height, 0)
			elseif opt.growth == "right" then
				bar:SetPoint("RIGHT", clcprotSovAnchor, "RIGHT", (i - 1) * (opt.width + opt.spacing), 0)
			else
				bar:SetPoint("TOP", clcprotSovAnchor, "TOP", opt.height / 2, (1 - i) * (opt.height + opt.spacing) )
			end
...
-- initialize the bars
function clcprot:InitSovBars()
	db = clcprot.db.profile	
...
function clcprot:CreateSovBar(index)
	local frame = CreateFrame("Frame", "clcprotSovBar" .. index, clcprotFrame)
	frame:Hide()
...
function clcprot:CreateSovAnchor()
	local frame = CreateFrame("Frame", "clcprotSovAnchor", clcprotFrame)
	frame:Hide()
...
function clcprot:ToggleSovAnchor()
	if self.showSovAnchor then
		-- hide
		self.showSovAnchor = false
		clcprotSovAnchor:Hide()
	else
		-- show
		self.showSovAnchor = true
		clcprotSovAnchor:Show()
	end
end

-- toggle it on and off
function clcprot:ToggleSovTracking()
