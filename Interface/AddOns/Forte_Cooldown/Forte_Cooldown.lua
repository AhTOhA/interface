...
function CD:RegisterCooldownPowerup(item,buff)	-- these are items
	tinsert(CD_SpecialCooldown,{"^"..(FW:ItemName(item)),FLAG_POWERUP,(FW:SpellName(buff))});
end
...
function CD:CheckCooldown(spell,start,duration,texture,flag)
	...
	local IgnoreCooldown = FW.Settings.IgnoreCooldown;
	local index;
	if flag == FLAG_BUFF or flag == FLAG_DEBUFF then
...
local function CD_TimeFilterRemaining(frame,t) 
	return (not frame.s.MinRemainingEnable or frame.s.MinRemaining <= t)
		and (not frame.s.MaxRemainingEnable or frame.s.MaxRemaining >= t); 
end

local function CD_TimeFilterDuration(frame,d) 
	return (not frame.s.MinRangeEnable or frame.s.MinRange <= d)
		and (not frame.s.MaxRangeEnable or frame.s.MaxRange >= d);
end
...
local function NewBar(parent,n)
	...
	bar.Update = function(self)
		...
		if s.SparkEnable then
			...
			self.icon.spark:SetAlpha(s.Spark);
...
function CD:CooldownGroupEnable()
...
FW:SetMainCategory(FWL.COOLDOWN_TIMER,FW.ICON.CD,4,"COOLDOWN","Cooldown","Cooldown");
	...
	FW:SetSubCategory(FWL.TIME_RANGE,FW.ICON.TIME,4);	
		...
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.MIN_DURATION,	FWL.MIN_DURATION_TT,	"MinRange",		CD_CooldownFilterChange,0);
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.MAX_DURATION,	FWL.MAX_DURATION_TT,	"MaxRange",		CD_CooldownFilterChange,0);
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.MIN_REMAINING,FWL.MIN_REMAINING_TT,	"MinRemaining",	CD_CooldownFilterChange,0);
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.MAX_REMAINING,FWL.MAX_REMAINING_TT,	"MaxRemaining",	CD_CooldownFilterChange,0);
	...
	FW:SetSubCategory(FWL.COLORING_FILTERING,FW.ICON.FILTER,5);
	...
	FW:SetSubCategory(FWL.MY_COOLDOWNS,FW.ICON.FILTER,5,nil,FW.REDUCED_ALPHA);
	...
	FW:SetSubCategory(FWL.BUFFS_DEBUFFS_ON_ME,FW.ICON.FILTER,5,nil,FW.REDUCED_ALPHA);	
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.BUFF,			FWL.CD_SPECIFIC7_TT,	"Buff", 		CD_CooldownFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FW.DIS_COL..FWL.ALL_OTHER_BUFFS,		"",				"BuffOther", 	CD_CooldownFilterChange);	
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.DEBUFF,				"",				"Debuff", 		CD_CooldownFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FW.DIS_COL..FWL.ALL_OTHER_DEBUFFS,	"",				"DebuffOther",	CD_CooldownFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.WEAPON_ENCHANT,		"",				"Enchant", 		CD_CooldownFilterChange);

	FW:SetSubCategory(FWL.SIZING,FW.ICON.SIZE,6);	
		...
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.ICON_SIZE,FWL.ICON_SIZE_TT,		"IconSize",		CD_CooldownShow,0);

	FW:SetSubCategory(FWL.APPEARANCE,FW.ICON.APPEARANCE,7);
...
FW:SetMainCategory(FWL.SECONDARY_SPLASH,FW.ICON.CD,4,"COOLDOWN","Splash","Splash");
	FW:SetSubCategory(FW.NIL,FW.NIL,1,FW.EXPAND);
...
FW:SetMainCategory(FWL.ADVANCED,FW.ICON.DEFAULT,99,"DEFAULT");
	FW:SetSubCategory(FWL.COOLDOWN_TIMER,FW.ICON.DEFAULT,4);
		FW:RegisterOption(FW.MS0,1,FW.NON,FWL.FRAME_LEVEL,FWL.FRAME_LEVEL_TT,	"CooldownStrata", CD_CooldownShow);
		FW:RegisterOption(FW.NUM,1,FW.NON,FWL.MIN_DURATION,	"",	"IgnoreCooldown",nil,2);
	FW:SetSubCategory(FWL.SECONDARY_SPLASH,FW.ICON.DEFAULT,4);
		FW:RegisterOption(FW.MS0,1,FW.NON,FWL.FRAME_LEVEL,FWL.FRAME_LEVEL_TT,	"SplashStrata", CD_SplashShow);
...
FW.Default.Cooldown.SparkEnable = true;
FW.Default.Cooldown.Spark = 1;
...
FW.Default.Cooldown.MinRemaining = 0;
FW.Default.Cooldown.MinRemainingEnable = false;
FW.Default.Cooldown.MaxRemaining = 3600;
FW.Default.Cooldown.MaxRemainingEnable = false;
...
FW.Default.Cooldown.BuffOtherColor =		{0.50,0.50,0.50};
FW.Default.Cooldown.BuffOtherEnable = false;
FW.Default.Cooldown.DebuffColor =	{1.00,0.00,0.00};
FW.Default.Cooldown.DebuffEnable = true;
FW.Default.Cooldown.DebuffOtherColor =	{0.50,0.00,0.00};
FW.Default.Cooldown.DebuffOtherEnable = false;
