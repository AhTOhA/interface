...
local function ScaleFunctionByThreatHigh(unit)
	if InCombatLockdown() and unit.reaction == "HOSTILE" then 
		if unit.type == "NPC" and unit.threatValue > 1 and unit.health > 0 then return LocalVars.ScaleSpotlight end
	end
end
...
local function ScaleFunctionByThreatLow(unit)
	if InCombatLockdown() and unit.reaction == "HOSTILE" then 
		...
		if unit.type == "NPC" and unit.health > 0 and unit.threatValue < 2 then return LocalVars.ScaleSpotlight end 
	end
end
