...
function TSM:OnEnable()
	...
	-- move the dealfinding info to the new table
	if TSM.db.global.DealfindingList then
		for _, data in pairs(TSM.db.global.DealfindingList) do
			local itemID, evenStacks, maxPrice = data.itemID, data.evenStack, data.price
			local name = GetItemInfo(itemID)
			TSM.db.profile.dealfinding[itemID] = {maxPrice=maxPrice, evenStacks=evenStacks, name=name}
		end
		TSM.db.global.DealfindingList = nil
	end
