...
function Automatic:LoadSidebar(parentFrame)
	...
	if TSM.db.profile.skipAutoModeIntro then
		Automatic.frame.goButton:Click()
	else
...
function Automatic:StartAutomaticMode()
	...
	for i, data in ipairs(list) do
		...
		if data.mode or TSM.db.global.neverShopFor[data.itemID] then
			...
			local destroyInfo = TSM.Destroying:GetDestroyableInfo(data.itemID)
			if destroyInfo == "prospect" and TSM.db.profile.noProspecting then
				destroyInfo = nil
			end
