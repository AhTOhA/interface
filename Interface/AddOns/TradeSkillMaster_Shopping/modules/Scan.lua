...
function Scan:AUCTION_ITEM_LIST_UPDATE()
	...
	if status.isScanning then
		...
		debug("scan2", status.filter.name or status.filter)
		Scan:ScanAuctions()
	end
end
...
function Scan:ScanAuctions()
	...
	if status.retries < 3 then
		...
		for i=1, shown do
			...
			local expectedName = type(status.filter) == "string" and status.filter or GetItemInfo(status.filter.itemID)
			if not strlower(name):match(strlower(expectedName)) then
				badData = true
			elseif not (quantity[i] and buyout[i]) then
				badData = true
			end
		end
	...
	for i=1, shown do
		...
		elseif type(status.filter) == "string" and strlower(GetItemInfo(itemID) or ""):match(strlower(status.filter)) then
			...
			debug("bad filter", status.filter.name or status.filter, status.filter.itemID or "nt", itemID)
		end
	end
...
function Scan:StopScanning(interrupted)
	...
	if interrupted == true then
		-- fires if the scan was interrupted (auction house was closed while scanning)
		TSM:Print(L["Scan interrupted due to auction house being closed."])
	elseif interrupted == "Automatic" then
		TSM:Print(L["Scan interrupted due to automatic mode being canceled."])
...
function Scan:ScanPageForAuction(auction)
	...
	if GetNumAuctionItems("list") > 0 then debug(TSMAPI:GetItemID(GetAuctionItemLink("list", 1))) else debug("0 items") end
