	...
	local page = {
		{
			...
			children = {
				...
				},
				{
					type = "InlineGroup",
					layout = "flow",
					title = L["Shopping - Milling / Disenchanting / Prospecting Options"],
					children = {
						{
							type = "Label",
							text = L["The options below apply to \"Shopping - Milling / Disenchanting / Prospecting\" searches (including those done while shopping for crafting mats)."],
							relativeWidth = 1,
						},
						{
							type = "HeadingLine",
...
function Config:LoadDealfindingOptions(container)
	local function AddItem(eb,_,item)
		item = item:trim()
		-- make sure the item is valid
		local ok, name, link = pcall(function() return GetItemInfo(item) end)
		if not (ok and name and link) then
		...
		TSM.db.profile.dealfinding[itemID] = {name=name, maxPrice=1}
		Config:UpdateTree()
...
function Config:LoadFolderAddRemove(container, folderName)
	...
	for itemID in pairs(TSM.db.profile.folders[folderName]) do
		...
		local iName = TSM.db.profile.dealfinding[itemID].name
		local iLink = link or iName
		tinsert(itemsInFolder, {value=itemID, text=iLink, icon=texture, name=iName})
	end
