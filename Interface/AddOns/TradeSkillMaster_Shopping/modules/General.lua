...
function General:ProcessCheapest(index)
	...
	if General.maxPrice then
		...
		if percent < 0 then
			extraText = "\n\n"..format("|cffff0000"..L["%s%% above maximum price."].."|r", abs(percent))
		else
			extraText = "\n\n"..format("|cff00ff00"..L["%s%% below maximum price."].."|r", percent)
		end
	end
	...
	General.frame.buyFrame.currentLabel:SetText(format(L["%s @ %s(%s per)"], link.."x"..quantity.."\n\n", TSM:FormatTextMoney(buyout) or "???", TSM:FormatTextMoney(cBuyout) or "???")..extraText)
