...
function Config:ShouldScan(itemID, isCancel)
	if TSMAuc.db.global.smartScanning then
		if Config:GetBoolConfigValue(itemID, "disabled") then
			return false
		end
		if isCancel and Config:GetBoolConfigValue(itemID, "noCancel") then
			return false
		end
	end
...
function Config:DrawGroupGeneral(container, group)
	...
	local function GetInfo(num)
		...
		elseif num == 6 then
			...
			elseif reset == "undercut" then
				return L["Auctions will be posted at a price such that they are undercutting the cheapest auction above your threshold when the market goes below your threshold."]
			end
		end
	end
				...
				},
				{
					...
					children = {
						...
						},
						{
							...
							value = TSMAuc.db.profile.minDuration[group] or TSMAuc.db.profile.minDuration.default,
							disabled = TSMAuc.db.profile.minDuration[group] == nil,
							disabledTooltip = overrideTooltip,
							callback = function(_,_,value) TSMAuc.db.profile.minDuration[group] = value container:SelectTab(1) end,
							onRightClick = function(self, value) SetGroupOverride("minDuration", value, self) end,
