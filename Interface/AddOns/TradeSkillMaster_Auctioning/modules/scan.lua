...
-- Add a new record
function Scan:AddAuctionRecord(name, link, owner, quantity, bid, buyout, timeLeft)
	...
	local minDuration = TSMAuc.Config:GetConfigValue(link, "minDuration")
	if quantity > ignoreStacksOver or quantity < ignoreStacksUnder then return end
	if timeLeft <= minDuration then return end
...
function Scan:ScanAuctions()
	...
	for i=1, shown do
		...
		Scan:AddAuctionRecord(name, TSMAuc:GetItemString(link), (owner or ""), quantity, bid, buyout, timeLeft)
	end
...
function Scan:GetPlayerAuctionCount(link, findBuyout, findBid)
	...
	for i=1, GetNumAuctionItems("owner") do
		...
		if wasSold == 0 and itemID == link and findBuyout == floor(buyout / stack) and findBid == floor(bid / stack) then
			quantity = quantity + 1
		end
	end
...
function Scan:GetLowestAuction(itemID)
	...
	for _, record in pairs(auctionData[itemID].records) do
		if not record.isPlayer and record.buyout == buyout then
			...
			if TSMAuc.db.factionrealm.blacklist[string.lower(record.owner)] then
				isBlacklist = true
			end
	...
	return buyout, bid, owner, isWhitelist, isBlacklist, isPlayer
end
...
function Scan:GetAuctions(itemID)
	if not auctionData[itemID] then return end
	local data = CopyTable(auctionData[itemID])
	sort(data.records, function(a, b) return a.buyout < b.buyout end)
	
	return data.records
end

-- -- used for the reset method of undercutting the lowest above threshold
-- function Scan:GetLowestWithinThreshold(itemID)
	-- local threshold = TSMAuc.Config:GetConfigValue(itemID, "threshold")
	-- if not auctionData[itemID] or not threshold then return end
	
	-- -- Find lowest within threshold
	-- local buyout, minDiff
	-- for _, record in pairs(auctionData[itemID].records) do
		-- local diff = (record.buyout or 0) - threshold
		-- if record.buyout and diff > 0 and diff < (minDiff or 1/0) then
			-- buyout = record.buyout
			-- minDiff = diff
		-- end
	-- end

	-- -- make sure the lowest isn't our own auction
	-- local isPlayer = true
	-- for _, record in pairs(auctionData[itemID].records) do
		-- if not record.isPlayer and record.buyout == buyout then
			-- isPlayer = false
