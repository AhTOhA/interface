...
local function SetExtendedFrameLevel(plate, level)
	extended = plate.extended
	extended:SetFrameLevel(level+1)
	extended.bars.healthbar:SetFrameLevel(level)
	extended.bars.castbar:SetFrameLevel(level)
end
...
do
	...
	local function GatherData_Alpha(plate)
		...
		if unit.isTarget then 
			...
			--SetExtendedFrameLevel(plate, 127)
			--extended:SetFrameLevel(127)
		--else
			--SetExtendedFrameLevel()
...
do
	...
	local function FindSmallestValue(tab)
		local prev, item = 1000000000, nil
	...
	local PlatePositionCache, PlateOrder = {}, {}
	local plateCount = 0
	local nextPlate, orderIndex
	
	-- OnUpdate: This function is processed every frame!
	function OnUpdate(self)
		HasMouseover = false
		-- Individual Plate Loop
		for plate in pairs(PlatesVisible) do
			...
			-- Vertical Sorting Init
			PlatePositionCache[plate] = plate:GetBottom()
			plateCount = plateCount + 1
		end
		
		-- Sort Loop
		for orderIndex = 1, plateCount do 
			nextPlate = FindSmallestValue(PlatePositionCache)
			--PlateOrder[orderIndex] = nextPlate
			if nextPlate then 
				PlatePositionCache[nextPlate] = nil 
				--nextPlate.extended:SetFrameLevel(max(1, 125 - (orderIndex*2)))
				SetExtendedFrameLevel(nextPlate, max(1, 125 - (orderIndex*2)))
			end
		end
		if currentTarget then 
			SetExtendedFrameLevel(currentTarget, max(1, 125))
			currentTarget.extended:SetFrameLevel(max(1, 125))
		end
